...
- Topbar compacta em Inter, sem serifa nem arco decorativo; no celular, CSV, PDF e Compartilhar ganham rótulo e a barra rola com a página, liberando altura útil.
- Cartão "hero" azul institucional com detalhe dourado para o valor estimado, no celular e no computador.
- Desktop em painel de duas colunas: lançamento à esquerda, planejamento do mês e tabela de valores à direita, lista em largura total.
- Resultado do lançamento calculado ao vivo também no computador, logo acima do botão Adicionar.
- Valor do cartão de métrica não é mais cortado em telas de 1280px.
- Cards de escala mais legíveis e cerca de 35% mais baixos: valor alinhado à direita, unidade visível, faixa lateral com a cor da situação e situação + ações numa linha só.
- Bottom sheet de lançamento com alça centralizada acima do título e botão fechar com alvo de 44px.
- Superfícies sólidas no lugar do efeito de vidro (backdrop-filter): mais contraste e menos custo de GPU.
- Modo escuro: marcadores da tabela de valores com contraste adequado.
- Correção: ícones da janela de Novidades voltam a ficar centralizados no quadro.
